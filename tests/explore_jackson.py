...
def explore():
    # Fetch Greg Jackson's episode Wikidata JSON data
    url = "https://www.wikidata.org/wiki/Special:EntityData/Q130466357.json"
    headers = {"User-Agent": "DesertIslandDiscsExplorer/1.0"}
    req = urllib.request.Request(url, headers=headers)
    
    print("Fetching Greg Jackson Episode Q130466357 JSON...")
    try:
        with urllib.request.urlopen(req, timeout=30, context=ssl_context) as response:
            data = json.loads(response.read().decode("utf-8"))
            
        entity = data.get("entities", {}).get("Q130466357", {})
        claims = entity.get("claims", {})
        
        print(f"Total claims found on episode Q130466357: {len(claims)}")
